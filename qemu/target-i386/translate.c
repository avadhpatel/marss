...
static void gen_simpoint_check_start(CPUX86State *env, DisasContext *dc)
{
    if (env->simpoint_decr) {
        ...
        tcg_gen_st_i64(count, cpu_env, offsetof(CPUX86State, simpoint_decr));
        tcg_temp_free_i64(count);
    }
}

static void gen_simpoint_check_end(CPUX86State* env, DisasContext *dc, int num_insns)
{
    if (env->simpoint_decr) {
        if (ptl_fast_fwd_enabled == 2 && dc->cpl != 3) {
            return;
