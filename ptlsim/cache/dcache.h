...
inline ostream& operator <<(ostream& os, const LoadStoreInfo& lsi) {
...
#ifdef L2_LINE_SIZE_64
  const int L2_LINE_SIZE = 64; //256;
  const int L2_SET_COUNT = 8192; // 256 KB
  const int L2_WAY_COUNT = 8;
#endif

#ifdef L2_LINE_SIZE_128
  const int L2_LINE_SIZE = 128; //256;
  const int L2_SET_COUNT = 4096; // 256 KB
  const int L2_WAY_COUNT = 8;
#endif

#ifdef L2_LINE_SIZE_256
  const int L2_LINE_SIZE = 256; //256;
//   const int L2_SET_COUNT = 2048; // 256 KB
  const int L2_SET_COUNT = 8192; // 256 KB
  const int L2_WAY_COUNT = 8;
#endif
...
#define DCACHE_L3_LINE_HITCOUNT_SLOTS      256
  ...
  struct LoadFillReqQueue {
    ...
    int add(const Entry& req);

    //    void wakeup(W64 address, const bitvec<LFRQ_SIZE>& lfrqmask);
    void wakeup(W64 address, const bitvec<size>& lfrqmask);

    void clock();

    Entry& operator [](int idx) { return reqs[idx]; }
    const Entry& operator [](int idx) const { return reqs[idx]; }

    ostream& print(ostream& os) const;
  };

  template <typename Entry, int size>
  static inline ostream& operator <<(ostream& os, const LoadFillReqQueue<Entry, size>& lfrq) {
  ...
  template<typename Entry, int SIZE>
  struct MissBuffer {
#if 0 //move out of the MissBuffer definition
    struct Entry {
      W64 addr;     // physical line address we are waiting for
      W16 state;
      W16 dcache:1, icache:1;    // L1I vs L1D
    ...
    int find(W64 addr);
    int initiate_miss(W64 addr, bool hit_in_L2, bool icache = 0, int rob = 0xffff, W8 threadid = 0xff);
    int initiate_miss(LoadFillReq& req, bool hit_in_L2, int rob = 0xffff);
    void annul_lfrq(int slot);
    void annul_lfrq(int slot, W8 threadid);
    void clock();
  ...
  template <typename Entry, int size>
  static inline ostream& operator <<(ostream& os, const MissBuffer<Entry, size>& missbuf) {
  ...
  struct CacheHierarchy {
    LoadFillReqQueue<LoadFillReq, LFRQ_SIZE> lfrq;
    MissBuffer<MissBufferEntry, MISSBUF_COUNT> missbuf;
    L1Cache L1;
    L1ICache L1I;
    L2Cache L2;
#ifdef ENABLE_L3_CACHE
    ...
    CacheHierarchy(W8 coreid_): coreid(coreid_), lfrq(*this), missbuf(*this) { callback = null; }
    ...
    W64 commitstore(const SFR& sfr, W8 threadid = 0xff, bool perform_actual_write = true);
    W64 speculative_store(const SFR& sfr, W8 threadid = 0xff);

    void initiate_prefetch(W64 addr, int cachelevel);

    bool probe_icache(Waddr virtaddr, Waddr physaddr);
    int initiate_icache_miss(W64 addr, int rob = 0xffff, W8 threadid = 0xff);

    void reset();
    void clock();
    void complete();
    void complete(W8 threadid);
