...
inline vec16b x86_sse_ldvbu(const vec16b* m) {
    vec16b rd; asm ("movdqu %[m],%[rd]" : [rd] "=x" (rd) : [m] "xm" (*m)); return rd;
}
inline void x86_sse_stvbu(vec16b* m, const vec16b ra) {
    asm ("movdqu %[ra],%[m]" : [m] "=xm" (*m) : [ra] "x" (ra) : "memory");
}
...
struct FullyAssociativeTagsNbitOneHot {
    ...
    base_t operator [](int index) const {
        return tagsmirror[index];
    }
...
struct LockableFullyAssociativeTags {
    ...
    int select_and_lock(T target) {
        bool dummy; return select_and_lock(target, dummy);
    }
    ...
    bool islocked(int way) const {
        return !unlockedmap[way];
    }
...
struct LockableFullyAssociativeArray {
    ...
    V* select_and_lock(T tag) {
        bool dummy; return select_and_lock(tag, dummy);
    }
...
struct LockableAssociativeArray {
    ...
    V* select_and_lock(T addr) {
        bool dummy; return select_and_lock(addr, dummy);
    }
...
struct DefaultCacheIndexingFunction {
    static inline Waddr setof(T address) {
        return bits(address, log2(linesize), log2(setcount));
    }
};
