...
static inline ostream& operator <<(ostream& os, const stringbuf& v) {
    stringbuf sb; sb << (char*)v; os.write((char*)sb, sb.size()); return os;
}
...
struct range {
    ...
    range<T>& operator ()(T lo, T hi) {
        this->lo = lo; this->hi = hi; return *this;
    }
    range<T>& operator ()(T v) {
        this->lo = v; this->hi = v; return *this;
    }
...
class dynarray {
    ...
    public:
        ...
        char* tokenize(char* string, const char* seplist) {
            abort();
        }
};

template <class T> static inline const T& operator <<(dynarray<T>& buf, const T& v) {
    return buf.push(v);
}
template <class T> static inline const T& operator >>(dynarray<T>& buf, T& v) {
    return (v = buf.pop());
}
...
struct RandomNumberGenerator {
    ...
    RandomNumberGenerator(W32 seed = 123) {
        reseed(seed);
    }
...
class queue : selflistlink {
    public:
        ...
        void add_to_head(selflistlink* link) {
            addlink(this, link, next);
        }
        void add_to_head(T& obj) {
            add_to_head(LM::linkof(&obj));
        }
        void add_to_head(T* obj) {
            add_to_head(LM::linkof(obj));
        }

        void add_to_tail(selflistlink* link) {
            addlink(prev, link, this);
        }
        void add_to_tail(T& obj) {
            add_to_tail(LM::linkof(&obj));
        }
        void add_to_tail(T* obj) {
            add_to_tail(LM::linkof(obj));
        }
...
struct bitvecbase {
    ...
    static size_t wordof(size_t index) {
        return index / BITS_PER_WORD;
    }
    static size_t byteof(size_t index) {
        return (index % BITS_PER_WORD) / __CHAR_BIT__;
    }
    static size_t bitof(size_t index) {
        return index % BITS_PER_WORD;
    }
    static T maskof(size_t index) {
        return (static_cast<T>(1)) << bitof(index);
    }

    T& getword(size_t index) {
        return w[wordof(index)];
    }
    T getword(size_t index) const {
        return w[wordof(index)];
    }
    T& hiword() {
        return w[N - 1];
...
struct bitvecbase<1> {
    ...
    void andop(const bitvecbase<1>& x) {
        w &= x.w;
    }
    void orop(const bitvecbase<1>& x) {
        w |= x.w;
    }
    void xorop(const bitvecbase<1>& x) {
        w ^= x.w;
    }
    void shiftleftop(size_t __shift) {
        w <<= __shift;
    }
    void shiftrightop(size_t __shift) {
        w >>= __shift;
    }
    void invertop() {
        w = ~w;
    ...
    bool equalop(const bitvecbase<1>& x) const {
        return w == x.w;
    }
    bool nonzeroop() const {
        return (!!w);
    ...
    size_t lsbop(size_t notfound) const {
        return (w) ? __builtin_ctzl(w) : notfound;
    }
    size_t msbop(size_t notfound) const {
        return (w) ? __builtin_clzl(w) : notfound;
    }
    void maskop(size_t count) {
        T m =
            (!count) ? 0 :
...
struct bitvecbase<0> {
    ...
    bool equalop(const bitvecbase<0>&) const {
        return true;
    }
    bool nonzeroop() const {
        return false;
    ...
    size_t nextlsbop(size_t, size_t) const {
        return 0;
    }
};
    ...
    public:
        class reference {
            ...
            public:
                ...
                bool testset() {
                    return x86_test_bts(*wp, bpos);
                }
                bool testclear() {
                    return x86_test_btr(*wp, bpos);
                }
                bool testinv() {
                    return x86_test_btc(*wp, bpos);
                }

                bool atomicset() {
                    return x86_locked_bts(*wp, bpos);
                }
                bool atomicclear() {
                    return x86_locked_btr(*wp, bpos);
                }
                bool atomicinv() {
                    return x86_locked_btc(*wp, bpos);
                }
        };
        ...
        bitvec(unsigned long long val) : base_t(val) {
            sanitize();
        }
        ...
        bitvec<N>& operator++(int postfixdummy) {
            return setall();
        }
        bitvec<N>& operator--(int postfixdummy) {
            return reset();
        ...
        bool operator ==(const bitvec<N>& rhs) const {
            return this->equalop(rhs);
        }
        bool operator !=(const bitvec<N>& rhs) const {
            return !this->equalop(rhs);
        }
        bool nonzero() const {
            return this->nonzeroop();
        }
        bool iszero() const {
            return !this->nonzeroop();
        }
        ...
        bitvec<N> operator <<(size_t shift) const {
            return bitvec<N>(*this) <<= shift;
        }
        ...
        size_t lsb(int notfound) const {
            return this->lsbop(notfound);
        }
        size_t msb(int notfound) const {
            return this->msbop(notfound);
        }
        size_t nextlsb(size_t prev, int notfound = -1) const {
            return this->nextlsbop(prev, notfound);
        }
...
struct HashtableKeyManager<W64, setcount> {
    ...
    static inline bool equal(W64 a, W64 b) {
        return (a == b);
    }
    static inline W64 dup(W64 key) {
        return key;
    }
...
struct FixedKeyValueHashtable {
    ...
    inline Tdata operator ()(Tkey key) {
        return get(key);
    }
    ...
    inline Tdata operator [](int i) const {
        return data[i];
    }
...
struct GenericChunkList {
    ...
    struct Iterator {
        ...
        Iterator(GenericChunkList<T>* chunk) {
            reset(chunk);
        }
        Iterator(GenericChunkList<T>& chunk) {
            reset(chunk);
...
struct SearchableChunkList16Entry {
    ...
    bool full() const {
        return (valid == bitmask(lengthof(list)));
    }
    bool empty() const {
        return (valid == 0);
    }
...
class CycleTimer {
    public:
        CycleTimer() {
            total = 0; tstart = 0; iterations = 0; title = "(generic)"; running = 0;
        }
        CycleTimer(const char* title) {
            total = 0; tstart = 0; iterations = 1; this->title = title; running = 0;
        }

        inline void start() {
            W64 t = rdtsc(); if (running) return; iterations++; tstart = t; running = 1;
        }
        inline W64 stop() {
            W64 t = rdtsc() - tstart;
...
struct CycleTimerScope {
    CycleTimer& ct;
    CycleTimerScope(CycleTimer& ct_) : ct(ct_) {
        ct.start();
    }
    ~CycleTimerScope() {
        ct.stop();
