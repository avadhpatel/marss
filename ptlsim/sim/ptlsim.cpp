...
// Global variables
//
PTLsimConfig config _read_mostly;
...
pthread_mutex_t translate_access;
...
CPUX86State* ptl_create_new_context() {
    ...
    if (config.threaded_simulation && ctx_counter == 0) {
        pthread_mutex_init(&qemu_access, NULL);

        pthread_mutexattr_t mattr;
        pthread_mutexattr_init(&mattr);
        pthread_mutexattr_settype(&mattr, PTHREAD_MUTEX_RECURSIVE);
        pthread_mutex_init(&translate_access, &mattr);

        int type;
        pthread_mutexattr_gettype(&mattr, &type);
        assert(type == PTHREAD_MUTEX_RECURSIVE);
