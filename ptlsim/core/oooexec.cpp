...
int ReorderBufferEntry::issue() {
  ...
  //  msdebug << " (uop.rc == REG_imm) ? ", (uop.rc == REG_imm), "  uop.rcimm ",  uop.rcimm, "  rc.data ",  (void*) rc.data, " rc ", rc, endl;
...
#ifdef WATTCH
//	if(isclass(uop.opcode, OPCLASS_FP_ALU) || isclass(uop.opcode, OPCLASS_FP_DIVSQRT))
...
#define USE_MSDEBUG logable(1000)
  ...
  if unlikely (aligntype == LDST_ALIGN_HI) {
    if likely (!annul) {
      ...
      //msdebug1 << " B: offset: ", offset, " sizeshift: ", sizeshift, " signext: ", signext, " data is ", (void*) data, endl;
    } else {
...
/*    if(covered){ // if we got from sfra:
      ...
      //    if unlikely (config.event_log_enabled) core.eventlog.add_load_store(EVENT_LOAD_HIT, this, sfra, addr);
      if unlikely (config.event_log_enabled) core.eventlog.add_load_store(EVENT_LOAD_FORWARD, this, sfra, addr);
...
//       lsi.signext = signext;    
//       // time stamp this load request:
      lsq->time_stamp = sim_cycle;
//       lsi.seq = lsq->time_stamp;
// //       SFR sfr;
// //       setzero(sfr);
// //       sfr = *lsq;
...
//       }
      ...
      bool L1hit = core.memoryHierarchy.access_cache(core.coreid,
                                                     ...
                                                     idx/*robid*/,
                                                     uop.uuid/* owner uuid */,
                                                     lsq->time_stamp/* owner time_stamp*/, 
                                                     physaddr, 
                                                     false/* icache */, 
                                                     false/* is write*/);
#else
  ...
  for (;;) {
    ...
    if unlikely (annulrob.lfrqslot >= 0) {
      ...
      if(!config.use_new_memory_system){
        ...
        assert(0);
        //        core.test_controller.annul_lfrq_slot(annulrob.lfrqslot);
      }
...
void ReorderBufferEntry::redispatch(const bitvec<MAX_OPERANDS>& dependent_operands, ReorderBufferEntry* prevrob) {
  ...
  if unlikely (lfrqslot >= 0) {
    if(!config.use_new_memory_system){
      ...
      assert(0);
      //      core.test_controller.annul_lfrq_slot(lfrqslot);
    }
    lfrqslot = -1;
  }
  ...
  if unlikely (lsq) {
    ...
    //    lsi_seq = 0; // reset to ignor issued miss
  }
