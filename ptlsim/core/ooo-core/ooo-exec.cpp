        ...
        if unlikely (!lock) {
            ...
            if(logable(8)) {
                cerr << "Memory addr ", hexstring(physaddr, 64),
                     " is locked by ", thread.ctx.env->cpu_index, endl;
...
#define SMT_ENABLE_LOAD_HOISTING
    ...
    if (!annul) {
        ...
        if(!core.memoryHierarchy->probe_lock(lockaddr, thread.ctx.env->cpu_index)) {
        ...
        if (uop.locked) {
            if(!core.memoryHierarchy->grab_lock(lockaddr, thread.ctx.env->cpu_index)) {
