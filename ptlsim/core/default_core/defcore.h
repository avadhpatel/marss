...
#define LSQ_SIZE (LDQ_SIZE + STQ_SIZE)
    ...
    // struct OutOfOrderCoreCacheCallbacks: public CacheSubsystem::PerCoreCacheCallbacks {
        // DefaultCore& core;
        // OutOfOrderCoreCacheCallbacks(DefaultCore& core_): core(core_) { }
...
#define InitClusteredROBList(name, description, flags) \
    ...
    struct ThreadContext {
        ...
        ThreadContext(DefaultCore& core_, W8 threadid_, Context& ctx_): core(core_), threadid(threadid_), ctx(ctx_) {
            reset();
        }
...
struct tlb_stat_tt { // rootnode: summable
    W64 hits;
    W64 misses;

    tlb_stat_tt& operator+=(const tlb_stat_tt &rhs) { // operator
        hits += rhs.hits;
        misses += rhs.misses;
        return *this;
    }
...
struct PerContextDefaultCoreStats { // rootnode:
    struct fetch {
    ...
    PerContextDefaultCoreStats& operator+=(const PerContextDefaultCoreStats &rhs) { // operator
        fetch += rhs.fetch;
...
// Out-of-Order Core
//
struct DefaultCoreStats { // rootnode:
    W64 cycles;
    ...
    PerContextDefaultCoreStats total;
    PerContextDefaultCoreStats vcpu0;
    PerContextDefaultCoreStats vcpu1;
    PerContextDefaultCoreStats vcpu2;
    PerContextDefaultCoreStats vcpu3;
    ...
    DefaultCoreStats& operator+=(const DefaultCoreStats &rhs) { // operator
