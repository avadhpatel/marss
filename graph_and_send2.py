...
#	chop_cmd = "tail --lines=+%d %s | head --lines=-%d | sed '/nan/d' > %s"%(skip_head,filename,skip_tail,output_filename);
...
if __name__ == "__main__":
	...
	bob_stats_file_name = "BOBstats.txt"
...
#TODO: save git diff output in the tgz
		last_req_str = Popen(["tail","-1",memlog_file_name],stdout=PIPE).communicate()[0]
		Popen(["tar","czf","run%d_%s_%d.tgz"%(posix_time,run_desc,run_num),memlog_file_name,memhist_file_name,log_file_base_name,"run%d.stats"%(run_num),"out.png"],stdout=PIPE).communicate()[0]
...
#			,SingleGraph(output_logfile,  "Bus Factors 2",[LinePlot(1,"busNumArbitrations","Bus Arbitrations")], x_axis_desc, AxisDescription("Arbitration Count (per 100k)"), col_to_num_map=header_map)
	...
	#	g.append(SingleGraph(bob_stats_file_name, "BOB pendingQueue Max",[LinePlot(1,4,"Num Entries")],  x_axis_desc, AxisDescription("# of Requests")))
